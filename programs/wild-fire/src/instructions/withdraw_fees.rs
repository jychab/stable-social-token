...
use anchor_spl::{
    ...
    token_interface::{
        transfer_checked, withdraw_withheld_tokens_from_mint, Mint, TokenAccount, TokenInterface,
        TransferChecked, WithdrawWithheldTokensFromMint,
    },
};
...
pub struct WithdrawFeesCtx<'info> {
    ...
    pub protocol_mint_token_account: Box<InterfaceAccount<'info, TokenAccount>>,
    #[account(
        mut,
        token::mint = mint,
    ...
    pub fee_collector_mint_token_account: Box<InterfaceAccount<'info, TokenAccount>>,
    #[account(
    ...
    transfer_checked(
        CpiContext::new(
            ...
            TransferChecked {
                ...
                mint: ctx.accounts.mint.to_account_info(),
                to: ctx.accounts.protocol_mint_token_account.to_account_info(),
                authority: ctx.accounts.authority.to_account_info(),
            },
        )
        .with_signer(signer),
